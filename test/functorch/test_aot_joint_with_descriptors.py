...
('call_function', 't_9', {'test': 1})""",
    ...
    def test_annotate_scope(self):
        class Bar(nn.Module):
            def forward(self, x):
                with fx_traceback.annotate_scope("bar"):
                    return x * 1
        ...
        class Foo(nn.Module):
            ...
            def forward(self, x):
                with fx_traceback.annotate_scope("foo"):
                    y = self.bar(x)
                    return y * 2
        ...
        class MyMod(nn.Module):
            ...
            def forward(self, x):
                with fx_traceback.annotate_scope("my_mod"):
                    x = x / 2
                    y = self.foo(x)
                return y - 1
        ...
        for with_export in [False, True]:
            graph_module = graph_capture(model, inputs, with_export)
            custom_metadata = fx_traceback._get_annotated_scope_metadata(graph_module)
            self.assertExpectedInline(
                str(custom_metadata),
                """\
